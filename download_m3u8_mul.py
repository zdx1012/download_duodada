...
class DownloadVideo:
    ...
    def __get_index_url(self, url):
        ...
        pat = re.compile(r'https://youku.cdn4-okzy.com/(\d*?)/(.*?)/index.m3u8')
        find_result = re.findall(pattern=pat, string=html)
        assert len(find_result) == 1, '未找到资源的起始播放链接'
        arg1, arg2 = find_result[0]
        self.__get_redirect_url(arg1, arg2)

    def __get_redirect_url(self, arg1, arg2):
        """
        获取重定向后的m3u8下载地址
        :param arg1:
        :param arg2:
        :return:
        """
        url = "https://youku.cdn4-okzy.com/{0}/{1}/index.m3u8".format(arg1, arg2)
        all_content = requests.get(url).text
        file_data = all_content.split("\n")
        new_url = parse.urljoin(url, file_data[2])
    ...
    def __download_ts(self, url):
        ...
        :return:
        """
        self.__save_dir = self.__base_dir + os.sep + r"download" + os.sep + self.__title  # 合并文件后的保存地址
        download_path = self.__base_dir + os.sep + r"download" + os.sep + self.__title + os.sep + self.__file_name
            ...
            try:
                for index, line in enumerate(file_line):
                    ...
                    if "EXTINF" in line:
                        unknow = False
                        # 拼出ts片段的URL
                        part_url = url.rsplit("/", 1)[0] + "/" + file_line[index + 1]
...
if __name__ == '__main__':
    video_url = "http://www.duodada.com/dianshiju/48615/player-2-31.html"
    video_title = '庆余年'
    qyn = DownloadVideo(start_url=video_url, title=video_title, download_all=True, base_dir=r'D:\BaiduNetdiskDownload')
    qyn.start_wodnload()
